...
app.get('/api/projects', authenticateToken, async (req: any, res) => {
    ...
    try {
        ...
        if (role === 'superadmin') {
            ...
            // HOD role - view projects in department
            const profile = await query('SELECT department_id FROM profiles WHERE id = $1', [id]);
            result = await query(baseSelect + ' AND p.department_id = $1 ORDER BY p.created_at DESC', [profile.rows[0].department_id]);
        } else {
            // PI, Co-PI, JRF, Assistant - view projects they are part of
            result = await query(`
                SELECT p.*,
                       json_build_object('name', d.name) as departments,
                       json_build_object('name', pr.name, 'email', pr.email) as profiles
                ...
                LEFT JOIN departments d ON p.department_id = d.id
                LEFT JOIN profiles pr ON p.pi_id = pr.id
                WHERE p.pi_id = $1 AND p.deleted_at IS NULL
...
app.get('/api/projects/:id', authenticateToken, async (req: any, res) => {
    try {
        ...
        if (userRole === 'superadmin') {
            ...
            // Check if user is PI or in team_members
        ...
        // Fetch team members and project years
        ...
        const logs = await query('SELECT al.*, p.name as user_name FROM activity_logs al LEFT JOIN profiles p ON al.user_id = p.id WHERE al.project_id = $1 ORDER BY al.created_at DESC LIMIT 50', [req.params.id]);
...
app.post('/api/projects/:id/team', authenticateToken, authorizeRoles('pi', 'co_pi', 'admin', 'superadmin'), async (req: any, res) => {
    const projectId = req.params.id;
    const { email, role, stipend } = req.body;
    ...
    try {
        ...
        if (profileResult.rows.length === 0) {
            ...
            const newProfile = await query(
                `INSERT INTO profiles (name, email, password, role) VALUES ($1, $2, $3, $4) RETURNING id`,
                [email.split('@')[0], email, tempPassword, role]
            );
